...
export const api = {
  ...
  addBranch: (body: { name: string; city?: string; username: string; password: string; kioskPin?: string }) =>
    req('/api/branches', { method: 'POST', body: JSON.stringify(body) }),
  updateBranch: (id: number, body: { name?: string; city?: string | null; lat?: number | null; lng?: number | null; radius?: number; workingDays?: number[]; kioskPin?: string }) =>

...
import { PageHead, Avatar, StatusChip, Field, type Tone } from '../ui'
...
export function EmployeeDetail() {
  ...
  if (!emp) {
    return (
      <div>
        <PageHead title="Çalışan kaydı" subtitle="Kayıt bulunamadı" />
        <div className="card" style={{ padding: 22, maxWidth: 640 }}>
          <div className="t-body ink-2" style={{ marginBottom: 14 }}>Çalışan kaydı yüklenemedi. Lütfen listeden tekrar seçin.</div>
          <button className="btn btn-primary" style={{ height: 42 }} onClick={back}>Çalışanlara dön</button>
        </div>
      </div>
    )
  }
  ...
  return (
    <div>
      <button className="btn btn-ghost" onClick={back} style={{ height: 36, padding: '0 10px', marginBottom: 12, gap: 6 }}>
        <Icon name="chevronL" size={18} color="var(--ink-2)" /> Çalışanlar
      </button>
      <PageHead title="Çalışan kaydı" subtitle={`SİCİL ${emp.sicil || '—'} · ${st[1]}`}
        actions={<>
      ...
      <div className="card col" style={{ padding: 22, gap: 14, maxWidth: 640 }}>
